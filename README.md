...
Import `runDecision<T>`, canonical decision types and validators, structured question builders, and `evaluateDecisionGate` from `@claritylabs/cl-sdk/decisions` in browsers and non-Node Convex modules. These primitives also remain exported from the root. See [structured decisions](DECISIONS.md) for adapters, policy controls, eligible callsites, and acceptance requirements.

Optional final text-evidence auditing is available through `@claritylabs/cl-sdk/extraction-audit` and `createExtractor`'s `evidenceAudit` result. It checks atomic document/profile facts and source-unit omissions in bounded batches, preserves reasoning repair, and binds diagnostics to the exact supplied snapshots. It defaults to legacy behavior and requires independent `extraction.audit` evaluation qualification for active use. Text auditing does not establish PDF visual completeness or authorize policy promotion. See [decision contracts and audit limits](./DECISIONS.md#final-bidirectional-evidence-audit).
