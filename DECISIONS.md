...
| `extraction.audit` | Final bidirectional audit of the merged, recovered, cleaned profile and materialized document in both v1/v2. Batches atomic support/contradiction checks with per-source-unit completeness checks. | One optional contextual reasoning repair, then reverify. Missing context, invalid citations, unsupported classes, normalization gaps and unreadable units remain unresolved. New evaluation qualification is required; the former pre-shard `extraction.verify` callsites are retired. |
...
Forward traversal recursively enumerates populated string/number/boolean leaves in both snapshots, including narratives, identities, dates, nested terms, schedules and financials. Null/empty values are not asserted facts. Citation IDs/hashes, infrastructure IDs, confidence, warnings and extraction bookkeeping are not semantic facts; they still affect the snapshot fingerprint. Meaningful agent-guidance titles, kinds and details are included. Every supplied source unit enters reverse traversal. The schema catalog includes absent fields and is derived from the schemas, rather than a small operational whitelist. Unsupported fact classes remain unresolved.

Questions share a complete compact global fact index. Each batch includes the complete supplied readable source context whenever it fits the byte limit, including across question-count partitions; byte limits partition questions before discarding source context. A single unit whose questions plus complete context cannot fit is unresolved without a semantic call. A source category and its speculative omission/contradiction/context questions share one request with independent forward questions. Only consumed branch uncertainty matters; consumed category choices require both confidence and selected-option probability to meet the configured threshold. All answers still require full contract validation. If the complete fact index or one source unit cannot fit, it is unresolved. Any unreadable source unit leaves the supplied context incomplete; readable-only support cannot establish policy-wide endorsement precedence. This conservative limitation can require reasoning on large documents; no savings or full-document semantic coverage is claimed.
