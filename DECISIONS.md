...
Questions share a complete compact global fact index. Each batch includes the complete supplied readable source context whenever it fits the byte limit, including across question-count partitions; otherwise it includes full untruncated local units and remains unresolved. A source category and its speculative omission/contradiction/context questions share one request with independent forward questions. Only consumed branch uncertainty matters; consumed category choices require both confidence and selected-option probability to meet the configured threshold. All answers still require full contract validation. If the complete fact index or one source unit cannot fit, it is unresolved. Local contexts cannot establish policy-wide endorsement precedence: any batch without the complete cross-unit text context records `cross_unit_context`, even if local support was confident. This conservative limitation can require reasoning on large documents; no savings or full-document semantic coverage is claimed.
