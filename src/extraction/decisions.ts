...
/** Verify each populated field and the completeness of the projection, then repair once. */
export async function verifyExtractionDecision<T>(params: {
  value: T;
  sourceSpans: SourceSpan[];
  decisions: ExtractionDecisionConfig;
  repair: (fields: string[]) => Promise<T>;
}): Promise<T> {
  const rule = params.decisions.decisionPolicy?.families?.["extraction.verify"];
  const mode = rule?.mode ?? params.decisions.decisionPolicy?.mode ?? "legacy";
  if (
    !params.decisions.decide ||
    mode === "legacy" ||
    (mode === "active" &&
      (!rule?.evaluationId || !rule.threshold || rule.threshold <= 0.5))
  )
    return params.value;
  const fields: Array<{ path: string; value: string | number | boolean }> = [];
  ...
  if (!fields.length)
    return mode === "active"
      ? params.repair(["missing operational facts"])
      : params.value;
  const flagged = new Set<string>();
  let failed = false;
  // Independent batches retain complete source context. No truncation or sampling certifies coverage.
  for (let start = 0; start < fields.length; start += 24) {
    ...
    batch.forEach((field, i) => {
      for (const [key, question] of Object.entries(
        verificationQuestions(decisionJson(field)),
      ))
        questions[`f${i}_${key}`] = question;
    });
    questions.omitted = {
      type: "noul",
      instructions:
        "Does the source evidence contain any important policy fact or coverage omitted from the extracted profile? Inspect the source, not just present keys.",
    };
    const passed = await runDecision({
      ...decisionOptions(params.decisions),
      family: "extraction.verify",
      state: decisionJson({
      ...
      accept: (answers) => {
        for (const [key, answer] of Object.entries(answers)) {
          const bad =
            answer.type !== "noul" ||
            (key.endsWith("supported") ? answer.noul < 0.5 : answer.noul > 0.5);
          if (bad)
            flagged.add(
              key === "omitted"
                ? "omitted facts / source coverage"
                : batch[Number(key.slice(1).split("_")[0])].path,
            );
        }
        return flagged.size ? undefined : true;
      },
      fallback: async () => false,
      onUsage: params.decisions.onDecisionUsage,
    });
    if (!passed) failed = true;
  }
  return mode === "active" && failed
    ? params.repair([...flagged])
    : params.value;
}
