...
function parseAnswer(value: unknown): DecisionAnswer {
  ...
  if (a.type === "noul") {
    ...
    if (
      Math.abs((values as number[]).reduce((sum, n) => sum + n, 0) - 1) > 0.001
    )
      invalid();
    if (a.type === "choice") {
      text(a.choice);
      if (
        !Object.prototype.hasOwnProperty.call(probs, a.choice) ||
        (probs[a.choice] as number) < Math.max(...(values as number[]))
      )
        invalid();
    } else {
      ...
      for (let i = 0; i < values.length; i++)
        if (
          !Object.prototype.hasOwnProperty.call(legend, String(i)) ||
          !Object.prototype.hasOwnProperty.call(probs, String(i))
        )
          invalid();
...
export function validateDecisionAnswers(
  ...
  for (const [id, q] of Object.entries(questions)) {
    if (!Object.prototype.hasOwnProperty.call(answers, id)) invalid();
    const a = parseAnswer(answers[id]);
    if (a.type !== q.type) invalid();
    if (a.type === "choice" && q.type === "choice") {
      if (
        Object.keys(a.probabilities).length !==
          Object.keys(q.criteria).length ||
        Object.keys(q.criteria).some(
          (key) => !Object.prototype.hasOwnProperty.call(a.probabilities, key),
        )
      )
        invalid();
    }
    if (a.type === "score" && q.type === "score") {
      if (
        ...
        q.criteria.some((entry, i) => !equalJson(entry, a.legend[String(i)]))
      )
        invalid();
    }
  }
