...
import { verifyQueryDecision } from "./decisions";
...
export interface VerifierConfig extends DecisionConfig {
  onDecisionUsage?: (usage?: TokenUsage) => void;
  generateObject: GenerateObject;
  providerOptions?: Record<string, unknown>;
  modelCapabilities?: ModelCapabilities;
  modelBudgetConstraints?: Partial<
    Record<ModelTaskKind, ModelBudgetConstraint>
  >;
}
...
export async function verify(
  ...
  return verifyQueryDecision({
    ...
    fallback: async () => {
      ...
      const deterministicIssues = deterministicQueryGroundingIssues(
        subAnswers,
        allEvidence,
      );
      if (deterministicIssues.length > 0) {
        return {
          result: {
            ...
            approved: false,
            issues: Array.from(
              new Set([...result.issues, ...deterministicIssues]),
            ),
            retrySubQuestions: Array.from(
              new Set([
                ...(result.retrySubQuestions ?? []),
                ...subAnswers
                  .filter((answer) =>
                    deterministicIssues.some((issue) =>
                      issue.includes(`"${answer.subQuestion}"`),
                    ),
                  )
