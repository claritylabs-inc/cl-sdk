...
| `insured_address` | `SourceBackedAddress` | Mailing address of the primary named insured, with source span provenance. See shared types appendix. | Declarations | high | `company_address` |
...
| `premium_amount` | `number \| null` | Total policy premium as a plain number when the source states a fixed amount. | Declarations / premium page | high | — |
...
| `total_cost_amount` | `number \| null` | Total amount due as a plain number when the source states a fixed amount. | Declarations / billing page | medium | — |
...
| `minimum_premium_amount` | `number \| null` | Minimum premium as a plain number when the source states a fixed amount. | Declarations / conditions | medium | — |
...
| `deposit_premium_amount` | `number \| null` | Deposit premium as a plain number when the source states a fixed amount. | Declarations | medium | — |
...
| `definitions` | `Definition[]` | First-class defined terms with full definition text, page number, form number/title, section reference, and source snippet. Used for policy terminology, coverage triggers, insured status, exclusions, duties, and other terms that affect interpretation. | Definitions / form text / endorsements | high | — |
| `covered_reasons` | `CoveredReason[]` | First-class affirmative coverage grants: covered causes of loss, covered losses, inclusions, qualifying reasons, covered events, and attached conditions/exceptions. Preserves coverage grouping and page/form provenance. | Insuring agreement / covered causes / what is covered / coverage descriptions | high | — |
...
| `coverages[].limit_amount` | `number \| null` | Coverage limit as a plain number when the source states a fixed numeric currency amount. | Declarations | high |
...
| `coverages[].deductible_amount` | `number \| null` | Deductible or retention as a plain number when the source states a fixed numeric currency amount. | Declarations | high |
...
Structured party and contact records are source-backed records. `insurer`, `producer`, `claims_contacts`, `regulatory_contacts`, `third_party_administrators`, `additional_insureds`, `loss_payees`, `mortgage_holders`, and `additional_named_insureds` must include non-empty `sourceSpanIds`.
...
**Storage system:** Glass `update_business_context` tool (see `src/prompts/application.ts`)
...
interface TaxFeeItem {
  ...
  amountValue?: number;    // Plain numeric amount when the source states a fixed amount
