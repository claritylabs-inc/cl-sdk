...
- `pdfBase64` and `images` — multimodal query/application payloads or non-extraction workflows when available

Current policy extraction uses source spans as the primary evidence path. The removed raw-PDF classify/page-map/focused-extractor flow is not part of `createExtractor()` in 3.2.13.
...
When source spans are provided, v3 extraction returns `sourceTree` and `operationalProfile`, and `chunks` is empty because broad structured chunks are no longer the primary evidence corpus. `document` remains a compatibility projection for hosts that still render `InsuranceDocument`. `PolicyOperationalProfile.declarationFacts` carries source-backed declaration facts such as named-insured mailing address, DBA, entity type, tax ID, and additional named insured rows for host persistence.
...
`OperationalCoverageSchedule` keeps a source-backed schedule name, `vehicle | property | location | other` kind, flexible items and label/value pairs, page range, and source span IDs. This allows partial or redacted schedules to remain useful without inventing fields required by stricter vehicle or location records.

`SourceBackedValue.value` preserves the extracted source-backed display value. For party and identity fields such as `namedInsured`, `insurer`, `broker`, and `policyNumber`, `normalizedValue` may contain the display-safe canonical identity after removing label fragments or address/contact tails. The compatibility `InsuranceDocument` projection prefers `normalizedValue` for those identity fields when present, while retaining raw `value` for dates, premium, and other fields where the extracted display format is meaningful.

`PolicyOperationalProfile.parties` is the policy-scoped owner for producer/broker, insurer/carrier, and MGA/administrator information. Party addresses may be partial in the profile; complete named-insured, producer, and insurer addresses are also projected into the compatibility document. `operationsDescription` must be directly supported by policy source IDs.
...
interface BaseDocument {
  ...
  // Insured details
  insuredDba?: string;
  insuredAddress?: SourceBackedAddress;
...
These types represent compatibility structured data on `InsuranceDocument`. On source-tree extraction paths, prefer `PolicyOperationalProfile` for product facts and `DocumentSourceNode` for policy wording/hierarchy.
...
type SourceBackedAddress = Address & SourceProvenance;
...
### Line of business and coverage classification

```typescript
type AcordLobCode =
  | "CGL" | "PROPC" | "AUTOB" | "WORK" | "UMBRC" | "EXLIA"
  | "EO" | "OLIB" | "EPLI" | "DO" | "FIDUC" | "CRIME"
  | "INMRC" | "COMAR" | "SURE" | "BOP" | "PROP"
  | "HOME" | "DFIRE" | "MHOME" | "AUTOP" | "UMBRP"
  | "FLOOD" | "EQ" | "INMRP" | "BOAT" | "RECV"
  | "CFRM" | "DISAB" | "UN"
  // plus the full accepted ACORD code list exported as ACORD_LOB_CODES.
...
All accepted ACORD line of business codes as a runtime array, useful for building prompts and validation. Unknown or unsupported legacy values normalize to `UN`; the unknown ACORD codes `ACHE`, `INTER`, `LICPT`, `LSTIN`, `NWFGR`, `PAPER`, `SCAP`, and `SFRNC` are intentionally excluded.

```typescript
const ACORD_LOB_CODES: AcordLobCode[];
```
