...
When `sourceSpans` are passed to `processApplication`, the pipeline forwards them to classification, field extraction, auto-fill, lookup, and explanation calls. This lets fields retain page or field-level evidence and gives downstream review surfaces a source-backed audit trail.
