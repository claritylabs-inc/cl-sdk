...
The query agent pipeline answers user questions against stored insurance documents and inbound multimodal context. It mirrors the extraction pipeline's deterministic scaffold plus agentic decision points: attachments are interpreted first, a classifier decomposes questions, a bounded planner decides whether retrieval is needed, reasoners answer from available evidence only, and a verifier checks grounding before composing the final response.
...
const agent = createQueryAgent({
  ...
  sourceRetriever,    // optional source-span retriever for quoted evidence
});
...
The pipeline runs six phases, with parallel dispatch in retrieval and reasoning when lookup is useful:

```mermaid
flowchart LR
  Q[User Question + Attachments] --> A[1. Interpret Attachments]
  A --> C[2. Classify]
  C --> P[3. Plan Retrieval]
  P --> N{Lookup needed?}
  N -->|yes| R[Retrieve]
  N -->|no| RE[4. Reason]
  R --> RE
...
- operations forwards an email body plus attachments into Glass
...
### Phase 3: Plan retrieval and retrieve when needed

For each sub-question, the planner checks classification flags and attachment evidence before launching storage work. If a question is general, attachment-only, or classified as not requiring document/chunk lookup, retrieval is skipped and the reasoner receives the available context directly.

When lookup is needed, retrievers run in parallel (concurrency-limited, default 3):
...
3. **Source retrieval** — optional source-span search for page-backed quoted evidence
4. **Conversation history** — search prior conversation turns for context
...
For each sub-question, a reasoner receives **only the evidence set** — attachment evidence, retrieved chunk/document/conversation evidence, or the explicit no-retrieval context selected by the planner. It never receives the full raw corpus. This forces grounding when evidence exists and keeps simple questions cheap. The reasoner produces a `SubAnswer` with:
...
If issues are found and the verify round limit hasn't been reached, the verifier triggers targeted retrieval/reasoning for flagged sub-questions instead of replaying the whole pipeline.
...
const agent = createQueryAgent({
  ...
  retrievalMode: "hybrid", // optional source retrieval strategy hint
