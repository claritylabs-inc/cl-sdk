...
- `PolicyOperationalProfile` is a source-backed projection for product-critical facts such as policy number, named insured, insured operations description, policy-scoped producer/insurer/MGA parties and addresses, policy period, coverage rows, limits, deductibles, premiums, and endorsement support.
...
Extraction builds a deterministic source tree from parser spans, promotes parser title elements into section/schedule nodes, and groups declarations, policy forms, and endorsements from source text rather than a separate form-inventory or source-tree-organizer model call. The hierarchy is content-first: declarations, policy forms, endorsements, sections, schedules, and tables are structure; pages are provenance. The primary operational-profile model extracts source-cited operational facts. When coverage recovery is enabled, a second semantic pass sketches every page, discovers relevant regions without depending on coverage-heading vocabulary, and processes those regions in bounded batches. It can add cited coverage terms, flexible asset schedules, and financial facts while rejecting unknown source IDs and unsupported numeric or identifier values. Query and PCE use retrievers to find source nodes, expand ancestors/siblings/children, and attach exact spans for citations. Application processing forwards spans into classification, field extraction, lookup, and explanation agents. Case workflows validate quoted evidence before proposals are scored or packets are generated.
...
- Do not reintroduce form-inventory or source-tree-organizer model calls. If grouping needs to improve, improve parser-span hierarchy and source-text recognition while keeping every retained node tied to real source spans.
