...
The pipeline runs twelve phases sequentially. Extraction, review follow-ups, and bounded optional work dispatch in parallel internally where useful.
...
After the page-mapped extractor tasks complete, the coordinator decides whether a `supplementary` pass is needed. This pass is intended for memory and retrieval rather than consumer-facing structured output.
...
After assembly, a formatting agent pass cleans up markdown in content-bearing string fields only when the content is likely to need it. Plain prose skips the model call. Long text, markdown artifacts, lists/headings, excessive spacing, and table-like content are batched for cleanup.
...
Content fields are collected and batched (up to 20 per model call) for efficient processing. The formatting prompt includes specific before/after examples for each pattern. Token usage from formatting is tracked and included in the total, but no-op formatting calls are avoided.
