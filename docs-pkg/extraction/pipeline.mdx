...
CL SDK's extraction pipeline uses an agentic coordinator/worker pattern to process insurance documents. It classifies the document, maps pages to focused extractors, builds deterministic tasks, dispatches focused extractors in parallel, merges repeated extractor runs, optionally reconciles merged artifacts, reviews for completeness and quality, and assembles a fully typed `InsuranceDocument` with retrieval-ready chunks.
...
The pipeline runs nine phases sequentially. Extraction and review dispatch work in parallel internally.
...
flowchart TD
    ...
    C --> D[3. Plan]
    D --> E[4. Extract + Merge]
    E --> R[5. Optional Reconcile]
    R --> F[6. Review]
    F -->|gaps found| E
    F -->|complete| G[7. Assemble]
    G --> H[8. Format]
    H --> I[9. Chunk]
    I --> J[InsuranceDocument + DocumentChunks]
```
...
If the same extractor runs multiple times, results merge instead of overwrite. This is especially important for `coverage_limits`, `endorsements`, `exclusions`, `conditions`, `sections`, and `declarations`.

### Optional reconciliation passes

After deterministic merging, the coordinator can run targeted reconciliation passes for artifacts that benefit from semantic consolidation. These passes are opt-in through `ExtractorConfig.reconciliation`.

The first built-in reconciler is:

- `reconciliation.exclusions` — sends the merged exclusions payload through a focused reconciliation prompt before review. This is intended for cases where deterministic merging leaves additive overlap, such as a base exclusion appearing separately from a carveback, exception, or narrower applicability clause.

This keeps the default pipeline predictable while allowing higher-recall cleanup where deterministic rules alone are not enough.

### The 11 focused extractors

| Extractor | Domain | What it extracts |
|-----------|--------|------------------|
| `carrier_info` | Carrier | Carrier name, NAIC, AM Best, MGA, underwriter, broker |
| `named_insured` | Insured | Insured name, DBA, address, entity type, FEIN, SIC/NAICS |
...
| `supplementary` | Supplementary | Regulatory context, contacts, TPA, claims contacts |
...
## Phase 5: Optional Reconciliation

If enabled, reconciliation runs after extraction/merge and again after any review-triggered follow-up extractors. Reconciliation operates on the merged artifact, not on raw page chunks, so it is narrowly scoped and cheaper than adding a second general extraction pass.

Today, exclusions are the main target because insurance forms often restate the same exclusion in slightly different ways across base forms and endorsements. The reconciler is expected to:

- combine semantically identical exclusions
- preserve additive exceptions, carvebacks, and applicability
- keep distinct form-specific exclusions separate when they are substantively different
...
interface ExtractorConfig {
  ...
  reconciliation?: { exclusions?: boolean };     // Optional post-merge reconciliation passes
}
```
