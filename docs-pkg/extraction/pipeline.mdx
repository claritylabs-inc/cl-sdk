...
CL SDK's extraction pipeline uses an agentic coordinator/worker pattern to process insurance documents. It classifies the document, maps pages to focused extractors, builds deterministic tasks, dispatches focused extractors in parallel, conditionally runs supplementary extraction for hidden retrieval facts, resolves referential coverage values, reviews for completeness and quality when configured, assembles a fully typed `InsuranceDocument`, promotes declarations data into typed top-level fields, generates a document summary, formats content in bounded parallel batches, and produces retrieval-ready chunks.
...
The page mapper receives chunked PDFs through `providerOptions.pdfBase64` or page-scoped Docling text through `providerOptions.doclingText`, then assigns pages one by one. Page-map chunks run with `pageMapConcurrency` when configured, otherwise the shared `concurrency` value.
...
The promoter runs eight sub-passes, all pure data transformations with no model calls:
...
| **Taxes and fees synthesis** | Converts declaration tax, fee, surcharge, and assessment fields into structured `taxesAndFees[]` entries, including common GST/HST/PST/QST-style labels. |
| **Premium and total-cost fallback** | If the premium extractor didn't find financial data, promotes premium and total-cost values from declaration aliases such as `premiumAmount`, `totalPaid`, `amountPaid`, and `amountCharged`. Tax fields are excluded from premium promotion. |
