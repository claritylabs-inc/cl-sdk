...
The page mapper receives chunked PDFs through `providerOptions.pdfBase64` and assigns pages one by one. Page-map chunks run with `pageMapConcurrency` when configured, otherwise the shared `concurrency` value.
