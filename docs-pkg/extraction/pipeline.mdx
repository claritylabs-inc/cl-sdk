...
CL SDK's extraction pipeline uses an agentic coordinator/worker pattern to process insurance documents. It classifies the document, maps pages to focused extractors, builds extraction tasks, dispatches focused extractors in parallel, conditionally runs supplementary extraction for hidden retrieval facts, resolves referential coverage values, reviews for completeness and quality when configured, assembles a fully typed `InsuranceDocument`, promotes non-financial declaration data into convenience top-level fields, generates a document summary, formats content in bounded parallel batches, and produces retrieval-ready chunks.
...
The pipeline runs twelve phases sequentially. Page mapping, extraction, review follow-ups, referential lookups, and formatting dispatch in parallel internally where useful.
...
flowchart TD
    ...
    C --> D[3. Plan]
    D --> E[4. Extract + Merge]
    E --> F{5. Supplementary signal?}
    F -->|yes| G[Page-scoped Supplementary]
    F -->|no| H[6. Resolve Referential]
    G --> H
    H --> I[7. Review or Deterministic Skip]
    I -->|gaps found| E
    I -->|complete| J[8. Assemble]
    J --> K[9. Promote]
    K --> L[10. Summarize]
    L --> M[11. Cost-aware Format]
    M --> N[12. Chunk]
    N --> O[InsuranceDocument + DocumentChunks]
```
...
Classification uses `generateObject` with the `ClassifyResultSchema` and drives template selection in the next phase. The coordinator passes the full PDF through `providerOptions.pdfBase64`, or host-provided Docling text through `providerOptions.doclingText`. See [Classification](/docs/extraction/classification) for details.

## Phase 2: Page Map

The coordinator first maps each page to one or more focused extractors with a model pass. This is the step that prevents broad, mixed ranges from sending declaration pages and generic policy forms through the same extractor without distinction.

The page mapper receives chunked PDFs through `providerOptions.pdfBase64`, page-scoped Docling text through `providerOptions.doclingText`, or caller-provided source-span text, then assigns pages one by one. Page-map chunks run with `pageMapConcurrency` when configured, otherwise the shared `concurrency` value.
...
The coordinator dispatches focused extractors in parallel (concurrency-limited, default 2). `extractorConcurrency` can be set independently when extractor calls should use a different provider lane than page mapping or formatting. Each extractor targets a specific data domain against the page range assigned by the plan.

Results accumulate in an in-memory `Map` keyed by extractor name.

Before each worker callback is invoked, the SDK slices the assigned page range and passes that page-scoped PDF through `providerOptions.pdfBase64`. If `convertPdfToImages` is configured, it passes `providerOptions.images` instead. For Docling inputs, the SDK skips PDF slicing and passes page-scoped `providerOptions.doclingText`. Full-document provider options, page-scoped PDFs, rendered images, Docling text ranges, and in-flight range requests are cached for the lifetime of one extraction so overlapping extractor tasks reuse local work.

If the same extractor runs multiple times, results merge instead of overwrite. This is especially important for `coverage_limits`, `definitions`, `covered_reasons`, `endorsements`, `exclusions`, `conditions`, `sections`, `declarations`, and `supplementary`.

Focused extractors can declare fallback behavior. Definitions and covered reasons fall back through section extraction when the focused extractor fails or returns no usable records, so a single no-output response does not leave those first-class fields empty.

### The 13 focused extractors
...
| `definitions` | Definitions | Defined terms, full definition text, form/page provenance |
| `covered_reasons` | Covered reasons | Covered causes of loss, covered losses, inclusions, qualifying reasons, attached conditions/exceptions |
...
## Phase 5: Conditional supplementary facts pass

The coordinator decides whether a `supplementary` pass is needed. This pass is intended for memory and retrieval rather than consumer-facing structured output.

The pass is skipped when there are no signals that hidden retrieval facts are present. It runs when page assignments, form inventory, existing extracted text, or review follow-up tasks indicate regulatory, claims, notice, cancellation/nonrenewal, contact, TPA, driver, household, lienholder, or schedule-row facts are likely present. Page/form signals are run as narrow page-scoped supplementary tasks in the main extractor batch; text signals discovered only after extraction fall back to a full-document supplementary pass.
...
## Phase 6: Resolve referential coverage values

Some coverage rows contain referential text such as "shown in the declarations", "see schedule", or "per endorsement". The resolver handles these values before review so downstream coverage fields are more useful.

Resolution is bounded. It first tries cheap local matches against already extracted sections and forms. Only unresolved high-value references can trigger targeted actions such as declaration lookup, schedule lookup, section lookup, page-location lookup, or skip. This avoids turning every vague form phrase into an expensive search chain.

## Phase 7: Review

After initial extraction, deterministic quality checks always run. The LLM review loop (up to `maxReviewRounds`, default 2) checks completeness and quality against the template's `required` fields when `reviewMode` allows it. The reviewer compares what has been extracted against what is expected, sees the document again, and receives a summary of extracted outputs and page assignments.
...
Typical quality issues include generic placeholders like "shown in declarations", "per schedule", values that look copied from generic form wording instead of declaration-specific schedules, mapped definitions pages with no definition records, and mapped covered-cause/inclusion pages with no covered reason records.

The reviewer receives the live extractor catalog, including registered fallback-capable extractors, so follow-up work can target definitions, covered reasons, supplementary facts, declarations, or section fallback extraction without relying on a hardcoded task list.

`reviewMode` controls the expensive LLM review pass:

| Mode | Behavior |
|------|----------|
| `"auto"` | Default. Runs LLM review unless deterministic checks are clean and source spans are available. |
| `"always"` | Always runs LLM review up to `maxReviewRounds`. |
| `"skip"` | Skips LLM review and relies on deterministic checks plus the configured `qualityGate`. |
...
After assembly, a promotion pass fills in non-financial convenience fields from data that was extracted but not directly surfaced. Numeric coverage and financial values remain owned by focused extractor and review LLM passes.

The promoter runs pure data transformations with no model calls:
...
Financial fields and numeric coverage fields are extracted by focused LLM passes and review follow-up tasks, not inferred in promotion from declaration aliases or coverage-name patterns. This keeps premium, total payable, taxes, fees, limits, deductibles, and payment-plan values tied to source-evidence extraction instead of hardcoded field-name matching.

All promotion sub-passes are idempotent — they never overwrite fields that already have values from direct extraction.
...
## Phase 11: Cost-aware format

After assembly, a formatting agent pass cleans up markdown in content-bearing string fields only when the content is likely to need it. Plain prose skips the model call. Long text, markdown artifacts, lists/headings, excessive spacing, and table-like content are batched for cleanup and processed with `formatConcurrency`.
...
Content fields are collected and batched (up to 20 per model call) for efficient processing. Batches run in parallel up to `formatConcurrency`. The formatting prompt includes specific before/after examples for each pattern. Token usage from formatting is tracked and included in the total, but no-op formatting calls are avoided.
...
Definitions and covered reasons produce dedicated `definition` and `covered_reason` chunks so retrieval can answer questions like "what does this term mean?" or "what losses are covered?" without depending on generic section text. Covered-reason conditions are chunked separately under the same `covered_reason` type.
...
interface ExtractorConfig {
  ...
  reviewMode?: "always" | "auto" | "skip";       // Default: "auto"
...
Your callback must treat `providerOptions` as part of the model input contract, not just optional metadata. In the extraction pipeline, the document reaches the model through `providerOptions.pdfBase64`, `providerOptions.images`, or `providerOptions.doclingText`.

### Tuning concurrency

The default concurrency of 2 balances throughput against rate limits. Use the shared `concurrency` value for simple setups, or split the lanes when your provider plan can handle more extractor work than formatting or page mapping:
