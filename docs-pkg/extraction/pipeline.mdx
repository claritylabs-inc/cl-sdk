...
CL SDK's extraction pipeline uses an agentic coordinator/worker pattern to process insurance documents. It classifies the document, maps pages to focused extractors, builds deterministic tasks, dispatches focused extractors in parallel, merges repeated extractor runs, reviews for completeness and quality, and assembles a fully typed `InsuranceDocument` with retrieval-ready chunks.
...
The pipeline runs eight phases sequentially. Extraction and review dispatch work in parallel internally.
