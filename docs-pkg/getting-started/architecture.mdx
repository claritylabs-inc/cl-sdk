...
The core of CL SDK is a source-tree pipeline that turns parser-provided insurance PDF spans into a canonical hierarchy plus source-backed operational facts. When source spans are available, the extractor builds deterministic document/page/table/row/cell nodes, runs an extraction-time organizer across every top-level page/form node with bounded child context, extracts only cited operational facts, and returns a compatibility `InsuranceDocument` projection for existing hosts.

```mermaid
flowchart LR
  PDF[Parser Source Spans] --> TREE[Deterministic Source Tree]
  TREE --> ORG[Bounded Organizer]
