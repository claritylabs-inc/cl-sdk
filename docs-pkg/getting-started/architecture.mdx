...
The core of CL SDK is a source-tree pipeline that turns parser-provided insurance PDF spans into a canonical hierarchy plus source-backed operational facts. When source spans are available, the extractor builds deterministic document/page/table/row/cell nodes, lets a bounded organizer label or group existing nodes, extracts only cited operational facts, and returns a compatibility `InsuranceDocument` projection for existing hosts.
...
The organizer prompt may label existing nodes and group adjacent nodes into forms, endorsements, schedules, clauses, or page groups. It cannot invent text, pages, source spans, or bounding boxes. If the organizer fails, the deterministic tree is still usable.
