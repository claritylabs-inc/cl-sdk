...
The core of CL SDK is a source-tree pipeline that turns parser-provided insurance PDF spans into a canonical hierarchy plus source-backed operational facts. When source spans are available, the extractor builds deterministic document/page/table/row/cell nodes, groups declarations, policy forms, endorsements, sections, and schedules from source text and parser titles, asks the configured model to extract only cited operational facts from a bounded evidence packet, and returns a compatibility `InsuranceDocument` projection for existing hosts.
...
Grouping is source-text driven rather than model-organizer driven. Separately numbered endorsements remain individual endorsement child nodes under a generic `Endorsements` page-group parent; extraction should not merge them into one endorsement node or use range titles such as `Endorsements 1-3`. Titles stay terse and source-heading-derived, such as `Declarations`, `Policy Form`, `Definitions`, `Endorsements`, or `Endorsement No. 3`, without parenthetical summaries. Generic table rows are kept as source nodes, but the operational-profile prompt only receives rows and spans with structural signals such as declaration item labels, section headings, premium rows, aggregate rows, or endorsement references.
