...
- classify, form inventory, and review calls include the full PDF in `providerOptions.pdfBase64`
- page mapping calls include page-scoped PDFs in `providerOptions.pdfBase64`
...
- Docling document inputs include full or page-scoped extracted text in the prompt and `providerOptions.doclingText`
- multimodal query calls may include `providerOptions.attachments`, `providerOptions.pdfBase64`, and `providerOptions.images`
- source-grounded workflows may include `providerOptions.sourceSpans` and `providerOptions.sourceChunks`
...
// Optional: provide source text spans for page-backed evidence
...
const { document, chunks, sourceChunks, tokenUsage, usageReporting } =
...
[cl-sdk] Extractor coverage_limits (pages 6-20) complete
