...
For extraction and multimodal query calls, `providerOptions` is also the document transport layer:
...
- `providerOptions.doclingText` carries full or page-scoped text normalized from a host-provided DoclingDocument
- `providerOptions.attachments` carries generic query attachments such as inbound photos, PDFs, and text snippets
- `providerOptions.sourceSpans` carries source evidence relevant to the current call
- `providerOptions.sourceChunks` carries grouped source evidence windows derived from spans
- classify and plan receive the full PDF, or Docling text when using Docling input
- page-map and worker extractors receive cached page-scoped PDFs, unless `convertPdfToImages` is enabled and cached page images are sent instead; Docling inputs receive page-scoped text
